...
class Frm_Reserva(Ui_Form):
    ...
    def search_patchworks(self) -> None:
        if self.txt_num_proj.text() != '':
            try:
                ...
                name_file = check_file_name(self.txt_num_proj.text(), first_name='PED')
    ...
    def select_list(self) -> list:
        ...
        for i in values:
            if i[0] == '+':
                value = [i[3], i[1], i[2], i[4], i[5], '', '', '', '', i[6], i[7], i[8], self.txt_num_proj.text()]
    ...
    def confirm_reserve(self) -> None:
        ...
        if result:
            self.exported_to_xls()
            message = "Operação Concluída !!!"
            Form.form_information(title, message)

    def exported_to_xls(self) -> None:
        wb = Workbook()
        ws = wb.active
        ...
        for i in list:
            table = 'tblRetalhos'
            column = 'Reserva'
            condition_condition = 'id'
            value_condition = i[1]
            string_connection = type_connection.config_connection(path)
            with ConnectionDB(string_connection) as db:
                db.update(
                    table, condition_condition, value_condition, {f"{column}" : f"{self.txt_num_proj.text()}"})    
            self.update_file(i[0], i[1])

        for k, row in enumerate(list):
            for v, item in enumerate(row[2:]):
                ws.cell(row=k+1, column=v+1, value=item)
